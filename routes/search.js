...
router.post('/', async (req, res) => {
  ...
  if (!config)
    return res.status(400).json({ message: `Invalid search type: "${type}". Valid types: ${Object.keys(SEARCH_TYPES).join(', ')}` });
  ...
  if (hasPlan) {
    ...
    if (user.dailySearches >= limit)
      return res.status(429).json({ message: `Daily limit (${limit}) reached. Resets at midnight.` });
  }
  ...
  await SearchLog.create({
    userId: user._id, userEmail: user.email, searchType: type,
    query: query.trim(), creditsUsed: hasPlan ? 0 : cost,
    success, resultLength: result ? JSON.stringify(result).length : 0, ipAddress: req.ip,
  });
